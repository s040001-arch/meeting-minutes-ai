...
def resolve_reader_blocking_findings(
    *,
    ...
    system = (
        ...
        "\n- 前後の文脈から確実に言える場合は、その内容へ直す。"
        "\n- 会話の崩れ断片（言いさし・相槌の混入・接続の破綻）は、意味と事実を"
        "変えない範囲で読みやすい形に整えてよい（2026-08-07 ユーザー決定）。"
        "言いさしの整理・冗長の削除・接続の補いは許可。新しい具体情報・"
        "推測の固有名詞は作らない。"
        "\n- 情報量のない崩れ片（宙に浮いた相槌・断ち切れた言いかけ）は削除してよい。"
        "\n- 主張・理由・事実は残し、人名・数値・固有名詞を変更・削除・追加しない。"
        "\n- 事実・数値・人名に関わるため判断できない場合のみ replacement を"
        "空文字にする。その項目は自動処理せず、内容を知る担当者へ質問として送られる。"
