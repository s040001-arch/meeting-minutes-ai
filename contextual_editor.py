...
def _build_system_prompt(meeting_profile: dict | None) -> str | list:
    ...
    static_prompt = (
        ...
        "\n- 候補が本当に出せないときだけ③。"
        "\n\n【人名誤認識の見落とし防止 — 参加者リストは不完全な前提で読む】"
        "\n- 【この会議について】の参加者欄はファイル名等からの推定であり、"
        "実際に発言・名指しされた人物が漏れていることがある。"
        "**参加者リストに無い名前だから無視してよい、とはならない**。"
        "\n- 日本語の人名として不自然な語（英字・カタカナの製品名/ブランド名等）に"
        "「さん/様/氏」が付き、人物として呼びかけ・依頼されている文脈なら、"
        "音声認識誤りの強い疑い。参加者リストの有無に関係なく②（文脈から候補が立てば）"
        "/③（立たなければ）で拾う。"
        "\n- 例: 「iPhone さんに〜お願いしても大丈夫ですか」→ 機材名が人物として扱われ依頼を"
        "受けている＝誤認識疑い。参加者欄に居なくても見送らない。"
        "\n\n【数値+単位の不整合 — 単位が珍しくても数値系として扱う】"
        "\n- 数字に直結する単位語（社/車/台/件/人/万円 等）が前後の文脈（直前の話題・"
        "会話の対象）と噛み合わない場合、単位そのものが誤認識の疑い。"
        "**正しそうな単位かどうかではなく、文脈の対象と一致するかで判断**。"
        "\n- 例: 子会社の数を話している文脈で「1000車」→ 助数詞「車」が文脈（社数）と"
        "不整合＝数値の誤認識疑い。fact_class は lexical_fluency にせず numeric とし、"
        "②（候補が立てば）/③で確認する。**数値系は同音っぽく見えても①では直さない**。"
