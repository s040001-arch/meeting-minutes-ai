...
**認証ポリシー: Google Docs API は OAuth 2.0 を使用する。サービスアカウントは個人 Google アカウントでは Docs API・Drive API ともに権限不足（403）となり使用不可。**

- [x] 実装: `railway_bootstrap.py` に `GOOGLE_SERVICE_ACCOUNT_JSON` → `credentials_service_account.json` の復元処理を追加
- [x] 実装: `railway_bootstrap.py` に `GOOGLE_OAUTH_TOKEN_JSON` → `token.json` の復元処理を追加
- [x] 実装: `railway_bootstrap.py` に `GOOGLE_OAUTH_CREDENTIALS_JSON` → `credentials.json` の復元処理を追加
- [x] 実装: `export_minutes_to_google_docs.py` の認証をサービスアカウントから OAuth に復元（2026-04-03）
- [x] 検証: Railway デプロイ後に OAuth トークン復元→Docs API 呼び出しが正常に動作することを確認（2026-04-10 再検証。2026-04-03 の検証はテストモード発行トークンだったため7日後に失効していた）
- [ ] 検証: Railway デプロイ後に Sheets API 認証（サービスアカウント）が正常に通ることを確認
...
### Step⑰ ナレッジ蓄積のパイプライン組み込み

- [x] 実装: `accumulate_knowledge_step17.py` を新設（バッチ蓄積専用モジュール）
  - `answers.json` を読み込み → Claude に全回答群をまとめて渡し統合判断 → Sheets 書き戻し
  - `knowledge_sheet_store.py` に `merge_all_answers_into_knowledge_store()` を追加
- [x] 実装: `run_job_once.py` の Step⑪完了直後（Step 6.2/6.3 後）に Step⑰ 呼び出しを追加
- [x] 実装: `run_resume_from_step7.py` の Step⑪相当完了直後にも同様に Step⑰ 呼び出しを追加
- [ ] 検証: ナレッジ蓄積が実行されること、かつ Knowledge Sheet に整合のとれた内容が書き戻されることを確認
- [ ] 検証: Step⑰ が失敗しても以降のパイプライン（Step⑫〜）が止まらないことを確認

### answers.json の設計と実装

- [x] 実装: ジョブディレクトリに `answers.json`（ジョブ内回答の累積ログ）を設計・定義
  - 形式: `[{"question_id": "...", "question_text": "...", "answer_text": "...", "answered_at": "..."}]`
- [x] 実装: `webhook_app.py` の回答受信時に `answers.json` へ追記（`_append_to_answers_json()`）
...
## OAuth 認証運用ガイド（2026-04-10 更新）

> **方針: Google Docs API は OAuth 2.0 を使用する。サービスアカウントは個人 Google アカウントでは Docs API・Drive API ともに権限不足（403）となり使用不可。**

### 前提

- GCP OAuth 同意画面は「本番」モード（2026-04-06 切替済み）。リフレッシュトークンは無期限有効。
- **注意**: 本番モード切替前に発行したトークンは7日で失効する。切替後にトークンを再発行すること。

### トークン再発行手順（リフレッシュ失敗時）

Railway ログに `RefreshError` / `invalid_grant` が出た場合、以下の手順でトークンを再発行する。

1. ローカルで `token.json` を削除
2. `python export_minutes_to_google_docs.py --job-id test --push --title test` を実行（仮の入力ファイルが必要）
3. ブラウザで Google 認可を完了
4. 生成された `token.json` の内容を Railway 環境変数 `GOOGLE_OAUTH_TOKEN_JSON` にコピー
5. Railway が再デプロイされ、新トークンで動作再開

### 実装状況

- [x] `railway_bootstrap.py`: `GOOGLE_OAUTH_TOKEN_JSON` → `token.json` の復元
- [x] `railway_bootstrap.py`: `GOOGLE_OAUTH_CREDENTIALS_JSON` → `credentials.json` の復元
- [x] `load_or_create_google_docs_credentials()`: リフレッシュ成功時に `token.json` を上書き保存
- [x] `load_or_create_google_docs_credentials()`: `refresh_token` があれば `expiry` 未設定でもリフレッシュを試行するよう修正（2026-04-10）
- [x] `load_or_create_google_docs_credentials()`: ヘッドレス環境で `InstalledAppFlow` に落ちず明確なエラーを出すよう修正（2026-04-10）
- [x] `run_job_once.py`: step_4_0（進捗 Doc 初期作成）を non-fatal に変更。失敗してもパイプライン続行（2026-04-10）
- [x] GCP OAuth 同意画面を「本番」に切替（2026-04-06）
- [x] 本番モード切替後にトークンを再発行し Railway 環境変数を更新（2026-04-10）
- [x] 検証: 再発行トークンで Railway 上の Docs API 呼び出しが正常動作することを確認（2026-04-10）

### 未実施

- [ ] 実装: パイプラインの OAuth 認証失敗時に LINE 通知を送る仕組みを検討
- [ ] 実装: Railway ログで `RefreshError` / `invalid_grant` を検知するアラート設定
...
- **2026-04-02③:** Step⑰ ナレッジ蓄積のパイプライン組み込みと answers.json を実装。`accumulate_knowledge_step17.py` 新設、`knowledge_sheet_store.py` にバッチ蓄積関数（`merge_all_answers_into_knowledge_store()`）追加、`webhook_app.py` に `_append_to_answers_json()` 追加。`run_job_once.py` および `run_resume_from_step7.py` の Step⑪完了後に Step⑰ 呼び出しを組み込み。
- **2026-04-03:** Google Docs API 認証をサービスアカウントから OAuth に戻す。`export_minutes_to_google_docs.py` の `_build_credentials()` を `load_or_create_google_docs_credentials()` に復元。`railway_bootstrap.py` に `GOOGLE_OAUTH_TOKEN_JSON`/`GOOGLE_OAUTH_CREDENTIALS_JSON` の復元処理を追加。OAuth リスク回避策セクションを新設。
- **2026-04-10:** OAuth トークン失効による step_4_0 連続失敗を修正。原因: テストモード中に発行したトークンの7日失効。対策: (1) `load_or_create_google_docs_credentials()` で `expiry` 未設定時もリフレッシュを試行するよう修正、(2) ヘッドレス環境で `InstalledAppFlow` に落ちず明確なエラーメッセージを出すよう修正、(3) step_4_0 を non-fatal に変更しパイプライン続行可能に、(4) 本番モード切替後にトークンを再発行し Railway 環境変数を更新。OAuth セクションを「運用ガイド」として全面改訂し、トークン再発行手順を明記。
