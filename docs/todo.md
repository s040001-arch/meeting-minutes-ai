...
**認証ポリシー: Google Docs API は OAuth 優先。サービスアカウントは個人アカウントでは不可（403 発生済み）。** 将来 Drive API 方式等のより優れた手段が確認できた場合のみ移行を検討してよい。
...
- [x] 検証: Railway デプロイ後に OAuth トークン復元→Docs API 呼び出しが正常に動作することを確認（2026-04-03）
...
## OAuth 認証リスク回避策（2026-04-03）

> **方針: Google Docs API は OAuth 2.0 を使用する（優先）。サービスアカウントは個人 Google アカウントでは権限不足（403）となり使用不可。** Drive API 経由 Docs 作成など、より優れた代替手段が検証済みになった場合のみ移行を検討してよい。

Google Docs API の認証をサービスアカウントから OAuth に戻した。
以下のリスクが存在し、対策を段階的に実施する。

### リスク1: テストモードでのリフレッシュトークン失効（7日間）

GCP OAuth 同意画面が「テスト」モードの場合、リフレッシュトークンは7日で失効する。

- [x] **最優先・手動作業**: GCP Console → OAuth 同意画面 → 「本番環境に公開」を実行する（2026-04-06 完了）
  - **これだけで7日失効問題が解消する**
  - 内部利用のみ（自分のアカウントのみ）なので Google 審査は不要
  - 本番化すればリフレッシュトークンの有効期限は無制限になる
- [ ] 検証: 本番モード切替後、7日以上経過してもトークン更新が正常動作することを確認

### リスク2: Railway（ヘッドレス環境）でのトークン再認可不可

Railway にはブラウザがないため、リフレッシュトークンが無効になった場合に再認可フローが実行できない。

- [x] 実装: `railway_bootstrap.py` で `GOOGLE_OAUTH_TOKEN_JSON` → `token.json` を復元する仕組みを追加
- [x] 実装: `railway_bootstrap.py` で `GOOGLE_OAUTH_CREDENTIALS_JSON` → `credentials.json` を復元する仕組みを追加
- [x] 実装: `load_or_create_google_docs_credentials()` でリフレッシュ成功時に `token.json` を上書き保存
- [ ] **運用**: リフレッシュ失敗ログが出た場合、ローカルで再認可→Railway 環境変数を更新する手順を確立
- [ ] 検証: Railway デプロイ後にトークン復元→リフレッシュ→Docs API 呼び出しが正常に動作することを確認
...
### 将来検討: Google Drive API 経由での Docs 作成

サービスアカウントでも利用可能な Drive API（MIME type conversion upload）方式へ移行すれば、
OAuth 依存を完全に排除できる。ただし現在は OAuth で安定動作しているため優先度は低い。

- [ ] 調査: Drive API の `files.create` で `mimeType=application/vnd.google-apps.document` を使った Docs 作成の可否と制約
- [ ] 実装: 必要に応じて `export_minutes_to_google_docs.py` を Drive API 方式に書き換え
