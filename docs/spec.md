# Meeting Minutes AI — spec.md (v2026-05)

---

## 0. Overview

Google Drive の固定フォルダに **Pixel レコーダー由来の `.txt`**（または互換テキスト）をアップロードすると、自動で AI 補正・不明点検出・議事録生成を行い、LINE を通じてユーザーと対話しながら議事録を完成させるシステム。

Railway 上で稼働し、Google Drive / Google Docs / Google Sheets / LINE Messaging API / Claude 4 Opus / Claude Sonnet を利用する。
...
- `drive_auto_run_forever.py` / `drive_auto_run_once.py` / `google_drive_poll_new_files.py`
- `build_job_id()` の job_id 生成（40 文字 sanitize 含む）
- Drive 検知〜移動〜 `run_job_once.py` 起動までのシーケンス

タイトル等の人間可読表示は **`run_job_once.py` 以降** で `display_title`（元ファイル名 stem）として再構築する。
...
対応拡張子:
- テキスト（本番）: `.txt`（Google Pixel レコーダーアプリ出力）
- 音声（検知のみ）: `.m4a`, `.mp3`, `.wav` — Drive 検知後パイプライン入口で exit 0（処理しない）
...
処理の進行に応じて Google Doc の **Drive ファイル名** を変更する。本体の H1 タイトルは `display_title`（元ファイル名から拡張子を除いた文字列・切り詰めなし）を用い、job_id は H1 に含めない。

| タイミング | Doc タイトル（Drive 名 / ステータス時） | 意味 |
|---------|-------------|------|
| 転写直後・Doc初期作成 | `【処理開始】{display_title}` | 生テキスト確認用（Step 4.0、`--docs-push` 時） |
| Step 4.2 機械補正 | `【機械補正中】{display_title}` | 機械補正実行中 |
| Step 4.3 AI補正 | `【AI補正中】{display_title}` | Claude 補正中 |
| Step 4.35 不明点検出完了 | `【不明点検出完了】{display_title}` | 不明点候補抽出完了 |
| Step 6 議事録生成 | `【議事録生成中】{display_title}` | 議事録セクション生成中 |
| Step 6.3 完了 | `{display_title}` | 確定状態（プレフィックスなし） |

`display_title` は `meeting_profile.json` の `display_title` フィールドに保存する。旧ジョブは `google_doc_hub.json` のタイトルまたは `e2e_run_log.txt` の `input_audio=` 行から推定する（`infer_display_title()`）。
...
| # | セクション名 | スタイル | 内容 |
|---|------------|---------|------|
| 1 | {display_title} | Heading 1（TITLE） | 元ファイル名 stem（job_id ではない） |
| 2 | 論点メモ | Heading 2 | 相原視点の論点整理（3〜7 項目） |
| 3 | 参加者 | Heading 2 | meeting_profile と一致、箇条書き |
| 4 | 議題 | Heading 2 | 箇条書き |
| 5 | 決定事項 | Heading 2 | 箇条書き |
| 6 | 残論点 | Heading 2 | 箇条書き |
| 7 | Next Action | Heading 2 | 箇条書き |
| 8 | 発言録（逐語） | Heading 2 | 話者ラベルなし |
| 9 | 管理情報 | Heading 2 | `job_id: {job_id}` の1行（末尾固定） |

生成元: `generate_minutes_other_sections.py` → `minutes_structured.md` → `export_minutes_to_google_docs.py`。

`run_docs_hub_e2e.py` は **デフォルトで compose をスキップ**し、上記 Markdown をそのまま Doc に反映する。レガシー再生成のみ `--compose-from-scratch` を指定（論点メモは含まれない旧形式）。
...
Step① Google Drive の監視フォルダ（DRIVE_FOLDER_ID）直下をポーリングし、新規ファイルを検出
       ...
       【リカバリフロー】ルート直下に新規ファイルがない場合:
         サブフォルダを作成日の古い順に走査し、
         「ソースファイル（音声/テキスト）はあるが Google Doc が未作成」の
         未完了ジョブを1件検出して Step④ から処理を再開する。
         この場合、Step②③（サブフォルダ作成・ファイル移動）はスキップされる
         （ファイルは既にサブフォルダ内にあるため）。
         未完了ジョブも見つからない場合のみ「処理対象なし」として終了する。
...
Step④ ファイルをローカルにダウンロードし、`run_job_once.py` を起動
       .txt の場合 → 内容を `merged_transcript.txt` にコピー（Whisper なし）
       非 .txt（音声）→ exit 0 で静かに終了
       filename_parser / meeting_profile でファイル名メタを構造化

Step⑤ テキストを merged_transcript.txt としてローカルのジョブディレクトリに保存
       meeting_profile.json に display_title・顧客・参加者等を保存

Step⑥（Step 4.0）Google Doc を新規作成（--docs-push 時）
       タイトル `【処理開始】{display_title}`、本文は生テキスト
       → Doc ID を google_doc_hub.json に保存
```

#### 注意事項

- **「ルート直下にファイルがない ≠ 処理済み」**: Step③ でファイルがサブフォルダへ移動した後もパイプラインは継続中。ルートが空でも処理が途中の場合がある。
- **自動リカバリ**: Step③ でファイルがサブフォルダへ移動済みだが後続処理（Step④〜⑥）が失敗した場合、次回のポーリングで自動的にリカバリフローが発動する。サブフォルダ内に「ソースファイルはあるが Google Doc が未作成」のジョブを検出し、Step④ から処理を再開する。手動介入は不要。
- **リカバリの判定基準**: サブフォルダ内の子ファイルを走査し、`mimeType=application/vnd.google-apps.document` が1つも存在しなければ未完了と判定する。対応するソースファイル（拡張子が `.m4a`/`.mp3`/`.wav`/`.txt`、または mimeType が `text/plain`/`audio/*`）が存在することも条件。
- **手動リカバリ（フォールバック）**: 自動リカバリで拾えない特殊ケースでは、`run_job_once.py --job-id <job_id> --input-audio <path>` を直接実行する。

### Phase B: 補正ループ（run_job_once.py / run_resume_from_step7.py）
...
Step 6.2  議事録セクション + 論点メモ（generate_minutes_other_sections.py）
Step 6.3  Google Doc 出力（export_minutes_to_google_docs.py）— H1/Drive 名 = display_title
Step 17   ナレッジ蓄積（meeting_profile の顧客・議題を Claude マージに渡す）
```

#### ジョブディレクトリ中間ファイル（デバッグ用）

| ファイル | 内容 |
|---------|------|
| `meeting_profile.json` | display_title, 顧客, 参加者, relevant_knowledge 等 |
| `unknown_points.json` | 不明点一覧（常時保存） |
| `minutes_sections_raw.json` | Step 6.2 Claude 生出力 JSON |
| `question_selection_audit.json` | Step 5 選定理由・value 式・proposal_impact |
| `question_result.json` | 質問選定結果（従来） |

#### 削除済みモジュール（2026-05）

`whisper_initial_prompt.py`, `ffmpeg_convert_to_wav.py`, `audio_split_chunks.py`, `transcribe_one_chunk.py`, `transcription_merge_chunks.py`, `diarize_speakers.py`, `transcript_postprocess.py`

---

### Phase B（旧記述・参考）

以下は旧 Step 番号の参考。実装は上記 Phase B に統合済み。
...
### context.json / meeting_profile.json

`context.json` は従来どおり Step 4.3 前後で更新される。

**meeting_profile.json**（2026-05 追加）— 全 Claude/GPT 呼び出しの共通コンテキスト:

```json
{
  "date": "2026-05-12",
  "customer_name": "エレマテック",
  "topic": "定年再雇用研修",
  "participants": ["相原", "臼井"],
  "meeting_scope": "external",
  "display_title": "2026_0512_エレマテック_定年再雇用研修_相原_臼井",
  "relevant_knowledge": ["..."],
  "job_context": { }
}
```

旧ジョブで `meeting_profile.json` が無い場合、`ensure_meeting_profile()` が hub / e2e ログから `display_title` を推定して生成する。

### context.json（ジョブ固有コンテキスト・従来）
...
| 入力 | Google Pixel レコーダー `.txt` |
| AI 補正・検出・分類・議事録生成 | Claude 4 Opus / Claude Sonnet (Anthropic API) |
| 質問選定 | OpenAI API（GPT） |
...
この状態は外部から見ると「処理対象がゼロ」に見えるが、実際には未完了ジョブが存在している。**ルート直下にファイルがないことは「全ジョブ完了」を意味しない。** この問題を解決するために `find_incomplete_job` によるリカバリフローを導入した（2026-04-03）。

---

## 15. 2026-05 デプロイ改修サマリー

| 項目 | 内容 |
|------|------|
| Phase 0 | 非 `.txt` は `run_job_once.py` で exit 0 |
...
| Fix ② | `run_resume_from_step7` に meeting_profile 渡し |
| Fix ③ | 不明点プロンプト：合意済み日程/数値除外、evidence 品質制約 |
| Fix ④ | デッドコード削除（whisper_initial_prompt 等） |
| Fix ⑤ | `minutes_sections_raw.json` / `question_selection_audit.json` 保存 |
| Fix ⑥ | `display_title` による H1/Drive 名統一、`## 管理情報` に job_id |

再生成: `regenerate_job_outputs.py`（`--skip-compose` 付き Docs 更新）、`run_resume_from_step7.py`。
