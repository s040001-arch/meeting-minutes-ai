...
| 2026-07-04 | Reader pass を Step 4.3r としてパイプラインに正式組み込み（`9ac6e85`）。`reader_pass.py`: Opus 4.8 で逐語録の論旨不明箇所を上位5件抽出・除外リスト（未回答②③）渡し・JSON出力。出力: `reader_pass_result.json` + `reader_pass_questions.md`（相原向け質問テンプレ）。`READER_PASS_ENABLED=on` で稼働（既定 off、Railway本番: on）。失敗時非致命的。テスト168件green。 |
| 2026-07-04 | 楽天インサイト社 質問→本文確定サイクル Step 2+3 完了（`79c8ff5`→`8b1f3e4`）。**TypeA pinpoint 8件**確定適用（大瀬→合瀬×2 / 人脈→戦略推進室 / 中本エマ→絵麻 / 坂本→鷹股(部長コンテキスト) / 制度高い→精度が高い / filler span 削除×2）。TypeB Opus再構成は**スパン範囲制御が不安定**（3件超過・3b revert 2件）で不採用、設計修正後に別途実装予定。`[補足:]` 5件は本文に保持したままStep3実行。Step3 reprocess(6.1→6.2→6.3) PASS: full_write_verified=True / Google Doc doc_chars=11,319 / [補足:]count=5 / 合瀬×24・戦略推進室×4・中本絵麻×1・鷹股×11 反映確認。`scripts/apply_rakuten_answers.py`: TypeA+TypeB apply ツール（再実行可能）。 |
| 2026-07-04 | **TypeB 再構成を rule-enforced で再適用**（`reconstruct_span.py`）。LLMは置換文字列のみ提案、境界・length/fact/semantic ガードはコード側。A-1（任意参加）・A-4（サーベイ主体）を適用、A-2/A-3/A-5は注釈stripのみ。相原GO後 Step3 reprocess: full_write_verified=True / Doc 読み戻し PASS（doc_chars=11,153 / [補足:]=0 / 合瀬×29・鷹股×13・戦略推進室×4・中本絵麻×1・参加は任意×1 / 旧誤0）。バックアップ: `*_backup_pre_apply_20260704_121811.txt`。 |
| 2026-07-04 | **質問→回答→本文確定→要約** の順序制御（`QUESTION_MODE`）。`off`（既定）=従来どおり。`on`/`cursor`=質問MD生成後に一時停止（exit 0、LINEなし）。`line`=波及バンドル1通をLINE送信して一時停止。再開: `reprocess_job.py --from-step resume`。波及グループ自動検出は `integrated_questions.py`（`question_bundle` 延長、全ジョブ共通）。 |
| 2026-07-04 | **`449818f`**: QUESTION_MODE 実装・波及削減・LINE準備を本番投入。`question_mode.py` / `integrated_questions.py` / `run_job_once` pause / `reprocess_job --from-step resume` / LINE bundle（`question_format=bundle`）。テスト **46 passed**。Railway production: `QUESTION_MODE=cursor`（Cursor 疑似運用、LINEなしで質問MDまで停止）。 |
| 2026-07-04 | reprocess_job.py 追加・楽天インサイト社に reader pass 注釈反映（`46cf272`）。`reprocess_job.py`: 完成済みジョブへ Step 6.1/6.2/6.3 を再適用する公式ツール（`--from-step` / `--input-override` / 自動バックアップ / `_reprocess_log.jsonl` 追記 / Step 6.3 後 Google Doc 読み戻し検証）。`readable_transcript.py`: `[補足: ...]` アノテーションを polish LLM から保護（`[要確認]` と同列）。初実戦: job_20260701_053826（楽天インサイト社）に reader pass 注釈5件を Google Doc まで反映・読み戻し count=5 確認済み。テスト191件green。 |
