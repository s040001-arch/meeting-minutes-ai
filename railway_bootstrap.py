...
def ensure_volume_seed_files() -> list[str]:
    """Volume が /app/data をマウントした場合でも seed から復元する。"""
    root = _repo_root()
    written: list[str] = []
    seeds = {
        os.path.join(root, "seed", "correction_dict.json"): os.path.join(
            root, "data", "correction_dict.json"
        ),
    }
    for src, dst in seeds.items():
...
if __name__ == "__main__":
    ...
    paths.extend(ensure_volume_seed_files())
    for p in paths:
