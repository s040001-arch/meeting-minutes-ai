...
【質問を増やさない原則】
- 代名詞の厳密な指示先や壊れた例示が不明でも、発言の要旨を変えずに
  一般化・省略できる場合は question_needed=false とし、replacement を出します。
- 例: 「それが何を学習するか」が曖昧でも「別のツールを使って学習機能を
  付けられる」という主張まで確実なら、その範囲に狭めて整文します。
- ユーザーへ質問するのは、固有名詞・数値・肯定否定・決定内容・発言主体など、
  一般化すると議事録上の重要事実が失われる場合だけです。
- 「より具体的に書けない」だけでは質問理由になりません。初見で要旨を理解でき、
  重要事実を誤らない表現にできるなら自動修正します。
