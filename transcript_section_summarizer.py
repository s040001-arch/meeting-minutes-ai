...
INTEGRATED_MIN_INPUT_CHARS = 400  # これ未満は見出し無し（挨拶だけの断片を除外）
...
def _build_integrated_system_prompt(meeting_profile: dict[str, Any] | None) -> str | list:
    ...
    static_prompt = (
        ...
        + "\n\n【分節ルール(改善A)】"
        "\n- 文字数ではなく、意味の塊で分ける。議題・論点・決定・宿題が切り替わる発言を境界にする"
        "\n  例: 接続確認→日程案の提示→日程変更の調整→前提条件・ボリュームの説明→宿題確認"
        "\n- 短い会議でも、意味が変われば切る。1セクションが200字でも構わない"
        "\n- 機械的な等間隔分割は禁止。字数の目安で切ってはいけない"
        "\n- 1セクションが極端に長い場合（おおよそ3,000字超）だけ、その中の次の意味境界で割る"
        "\n\n【見出し(summary)ルール(改善B)】"
        "\n- 15〜30字の体言止め、名詞句中心"
